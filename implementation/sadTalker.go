...
func (a *SadTalker) postData(url string, data []byte) (error, string) {
	funName := "postData"
	client := &http.Client{
	...
	request.Header.Add("Authorization", SadTalkerWorkSpaceId)
...
func (a *SadTalker) GetData(url string, taskId string) (error, string) {
	funName := "GetData"
	now := time.Now().Unix()
	defer func() {
	...
	log4plus.Info("%s url=[%s] client.Do", funName, url)
	responseBody, err := ioutil.ReadAll(response.Body)
	if err != nil {
		errString := fmt.Sprintf("%s ioutil.ReadAll Failed url=[%s] err=[%s]", funName, url, err.Error())
		log4plus.Error(errString)
...
func (a *SadTalker) SadTalker(method, apiKey string, data []byte) (error, string) {
	funName := "SadTalker"
	now := time.Now().Unix()
	defer func() {
	...
	url := configure.SingtonConfigure().Interfaces.SadTalker.Urls.SadTalker.MethodUrl
	...
	if int(result.ResultCode.(float64)) < 0 {
		...
		log4plus.Error(errString)
		return err, errString
	} else if int(result.ResultCode.(float64)) != 200 {
		errString := fmt.Sprintf("%s int(result.ResultCode.(float64)) = [%d]", funName, int(result.ResultCode.(float64)))
		log4plus.Error(errString)
		return err, errString
	} else {
...
func (a *SadTalker) pollTasks() {
	funName := "pollTasks"
	for {
		time.Sleep(time.Duration(5) * time.Second)

		methodNames := []string{"sadTalker"}
		...
		for _, v := range tasks {
			...
			err, resultData := a.GetData(configure.SingtonConfigure().Interfaces.SadTalker.Urls.SadTalker.GetTask, v.TaskID)
			if err != nil {
...
func SingtonSadTalker() *SadTalker {
	if nil == gSadTalker {
		gSadTalker = &SadTalker{}
		go gSadTalker.pollTasks()
	}
	return gSadTalker
}

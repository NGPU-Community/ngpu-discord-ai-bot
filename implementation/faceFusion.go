...
type ResponsePostDataInterface struct {
...
func (a *FaceFusion) postData(url string, data []byte) (error, string) {
	funName := "postData"
	client := &http.Client{
	...
	request.Header.Add("Authorization", FaceFusionWorkSpaceId)
...
func (a *FaceFusion) GetData(url string, taskId string) (error, string) {
	funName := "GetData"
	client := &http.Client{
		Transport: &http.Transport{
			Dial: func(netw, addr string) (net.Conn, error) {
...
func (a *FaceFusion) BlackWhite2Color(method, apiKey string, data []byte) (error, string) {
	...
	//提交生成音频文件
	url := configure.SingtonConfigure().Interfaces.FaceFusion.Urls.BlackWhite2Color.MethodUrl
...
func (a *FaceFusion) LipSyncer(method, apiKey string, data []byte) (error, string) {
	funName := "LipSyncer"
	now := time.Now().Unix()
	defer func() {
		log4plus.Info("%s consumption time=%d(s)", funName, time.Now().Unix()-now)
	}()
	url := configure.SingtonConfigure().Interfaces.FaceFusion.Urls.LipSyncer.MethodUrl
	requestTime := time.Now()
...
func (a *FaceFusion) FaceSwap(method, apiKey string, data []byte) (error, string) {
	funName := "FaceSwap"
	now := time.Now().Unix()
	defer func() {
		log4plus.Info("%s consumption time=%d(s)", funName, time.Now().Unix()-now)
	}()
	url := configure.SingtonConfigure().Interfaces.FaceFusion.Urls.FaceSwap.MethodUrl
	requestTime := time.Now()
...
func (a *FaceFusion) FrameEnhance(method, apiKey string, data []byte) (error, string) {
	funName := "FrameEnhance"
	now := time.Now().Unix()
	defer func() {
		log4plus.Info("%s consumption time=%d(s)", funName, time.Now().Unix()-now)
	}()
	url := configure.SingtonConfigure().Interfaces.FaceFusion.Urls.FrameEnhance.MethodUrl
	requestTime := time.Now()
...
type ResponseGetDataRsultInterface struct {
	Data interface{} `json:"data"`
}

func (a *FaceFusion) pollTasks() {
	funName := "pollTasks"
	for {
		time.Sleep(time.Duration(5) * time.Second)

		methodNames := []string{"blackWhite2Color", "lipSyncer", "faceSwap", "frameEnhance"}
		...
		for _, v := range tasks {
			...
			getTaskUrl := ""
			if strings.ToLower(v.Method) == strings.ToLower("blackWhite2Color") {
				getTaskUrl = configure.SingtonConfigure().Interfaces.FaceFusion.Urls.BlackWhite2Color.GetTask
			} else if strings.ToLower(v.Method) == strings.ToLower("lipSyncer") {
				getTaskUrl = configure.SingtonConfigure().Interfaces.FaceFusion.Urls.LipSyncer.GetTask
			} else if strings.ToLower(v.Method) == strings.ToLower("faceSwap") {
				getTaskUrl = configure.SingtonConfigure().Interfaces.FaceFusion.Urls.FaceSwap.GetTask
			} else if strings.ToLower(v.Method) == strings.ToLower("frameEnhance") {
				getTaskUrl = configure.SingtonConfigure().Interfaces.FaceFusion.Urls.FrameEnhance.GetTask
			} else {
				continue
			}

			err, resultData := a.GetData(getTaskUrl, v.TaskID)
			if err != nil {
			...
			if aiModule.ResultCode > 0 && aiModule.ResultCode != 101 {
				curTime := time.Now()
...
func SingtonFaceFusion() *FaceFusion {
	if nil == gFaceFusion {
		gFaceFusion = &FaceFusion{}
		go gFaceFusion.pollTasks()
	}
	return gFaceFusion
}
